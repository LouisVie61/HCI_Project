...
export const Lessons = () => {
  ...
  return (
    ...
    >
      ...
      {progressError && <NoticeState tone="danger" title="Không tải được tiến độ" message={progressError} />}
      {!loading && !error && lessons.length === 0 && (
        <NoticeState tone="neutral" title="Không có bài học phù hợp" message="Thử đổi từ khóa, độ khó hoặc trạng thái học." />
      )}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {lessons.map((lesson, index) => {
          ...
          const progressLabel =
            progressPercent >= 100 ? 'Hoàn thành' : progressPercent > 0 ? 'Đang học' : 'Chưa học';
            ...
            <article key={lesson.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                ...
                <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">
                  {progressLoading ? 'Đang tải tiến độ...' : `${progressLabel} · ${progressPercent}%`}
                </span>
              </div>
